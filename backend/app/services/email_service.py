...
class EmailService:
    ...
    async def get_active_service(self):
        """Возвращает активный сервис (Resend приоритетнее)."""
    ...
    async def send_email(
        ...
        if not service:
            ...
            return False
        return await service.send_email(to_email, subject, html_content, text_content)
    ...
    async def send_verification_code(self, to_email: str, code: str) -> bool:
        ...
        if hasattr(service, 'send_verification_code'):
            return await service.send_verification_code(to_email, code)
